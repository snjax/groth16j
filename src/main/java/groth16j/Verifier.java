...
class VK {
  ...
  public static VK create(byte[] vk) {
    ...
    for (int i = 14; i < wordlen; i += 2) {
      BN128G1 ICt = parseBN128G1(vk, i);
      if (ICt == null)
        return null;
      IC[(i - 14) / 2] = ICt;
    }
    BN128G1 alpha1 = parseBN128G1(vk, 0);
    BN128G2 beta2 = parseBN128G2(vk, 2);
    BN128G2 gamma2 = parseBN128G2(vk, 6);
    BN128G2 delta2 = parseBN128G2(vk, 10);
    if (alpha1 == null || beta2 == null || gamma2 == null || delta2 == null)
      return null;
    else
      return new VK(alpha1, beta2, gamma2, delta2, IC);
...
class Proof {
  ...
  public static Proof create(byte[] proof) {
    BN128G1 A = parseBN128G1(proof, 0);
    BN128G2 B = parseBN128G2(proof, 2);
    BN128G1 C = parseBN128G1(proof, 6);
...
public class Verifier {
  ...
  private static boolean prod4(BN128G1 a1, BN128G2 a2, BN128G1 b1, BN128G2 b2, BN128G1 c1, BN128G2 c2, BN128G1 d1,
      BN128G2 d2) {
  ...
  private static boolean verify(VK vk, Proof proof, BigInteger[] input) {
    ...
    return prod4(new BN128G1(proof.A.negate()), proof.B, vk.alpha1, vk.beta2, new BN128G1(vk_x), vk.gamma2, proof.C,
        vk.delta2);
  }

  public static boolean verify(byte[] _vk, byte[] _proof, byte[] _input) {
    if (_vk.length % 64 != 0 || _proof.length != 256 || _input.length % 32 != 0
        || (_vk.length / 64 - 8) != (_input.length / 32))
      return false;

    VK vk = VK.create(_vk);
    Proof proof = Proof.create(_proof);
    if (vk == null || proof == null)
      return false;
